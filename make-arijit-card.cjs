...
async function buildFront() {
  ...
  <text x="60" y="320" font-family="Georgia, serif" font-size="58" font-weight="bold" fill="${WHITE}">Arijit Sarmah</text>
  <text x="62" y="362" font-family="Calibri, Arial, sans-serif" font-size="30" letter-spacing="2" fill="${SAND}">Advisor</text>
  <text x="60" y="450" font-family="Calibri, Arial, sans-serif" font-size="34" font-weight="bold" fill="${WHITE}">WhatsApp  +91 99786 70073</text>
  <text x="60" y="500" font-family="Calibri, Arial, sans-serif" font-size="30" fill="${SAND}">storeyinfra.com</text>
  </svg>`
  const buf = await sharp(Buffer.from(svg)).jpeg({ quality: 95 }).toBuffer()
  fs.writeFileSync('C:\\consne\\arijit-sarmah-visiting-card-front.jpg', buf)
  return buf.length
}
...
async function buildBack() {
  ...
  <text x="520" y="262" font-family="Georgia, serif" font-size="56" font-weight="bold" fill="${TERRA}">save contact</text>
  <text x="522" y="330" font-family="Calibri, Arial, sans-serif" font-size="30" fill="${COCOA}">Point your camera at the code —</text>
  <text x="522" y="370" font-family="Calibri, Arial, sans-serif" font-size="30" fill="${COCOA}">your phone adds the contact.</text>
  ...
  const buf = await sharp(Buffer.from(svg)).jpeg({ quality: 95 }).toBuffer()
  fs.writeFileSync('C:\\consne\\arijit-sarmah-visiting-card-back.jpg', buf)
  return buf.length
}
...
async function buildPPTX() {
  ...
  f.addText('Arijit Sarmah', { x: 0.2, y: 0.92, w: 3.1, h: 0.32, fontFace: 'Georgia', fontSize: 20, bold: true, color: 'FFFFFF' })
  f.addText('Advisor', { x: 0.21, y: 1.22, w: 3.1, h: 0.2, fontFace: 'Calibri', fontSize: 11, color: 'E7E8D1' })
  f.addText('WhatsApp  +91 99786 70073', { x: 0.2, y: 1.5, w: 3.1, h: 0.22, fontFace: 'Calibri', fontSize: 12, bold: true, color: 'FFFFFF' })
  f.addText('storeyinfra.com', { x: 0.2, y: 1.72, w: 3.1, h: 0.2, fontFace: 'Calibri', fontSize: 10, color: 'E7E8D1' })
  ...
  b.addText('Scan to save contact', { x: 1.7, y: 0.45, w: 1.65, h: 0.7, fontFace: 'Georgia', fontSize: 17, bold: true, color: 'B85042' })
  b.addText('Point your camera at the code — your phone adds the contact.', { x: 1.7, y: 1.12, w: 1.65, h: 0.5, fontFace: 'Calibri', fontSize: 9, color: '2A1410' })
  b.addText('storeyinfra.com', { x: 1.7, y: 1.65, w: 1.65, h: 0.2, fontFace: 'Calibri', fontSize: 9, color: '6B5750' })

  await pptx.writeFile({ fileName: 'C:\\consne\\arijit-sarmah-visiting-card.pptx' })
}

async function main() {
  const fLen = await buildFront()
  const bLen = await buildBack()
  console.log('FRONT:', (fLen/1024).toFixed(0), 'KB  → arijit-sarmah-visiting-card-front.jpg')
  console.log('BACK :', (bLen/1024).toFixed(0), 'KB  → arijit-sarmah-visiting-card-back.jpg')
  await buildPPTX()
  console.log('PPTX : arijit-sarmah-visiting-card.pptx')
}
